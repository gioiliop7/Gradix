<nav class="navbar navbar-default navbar-uth">
	<div class="container containeruth">
		<div class="navbar-header">
			<button type="button" class="navbar-toggle collapsed " data-toggle="collapse" data-target="#bs-example-navbar-collapse-1" aria-expanded="false">
				<span class="sr-only">Toggle navigation</span>
				<span class="icon-bar"></span>
				<span class="icon-bar"></span>
				<span class="icon-bar"></span>
			</button>
			<a class="navbar-brand" href="./nps.php">
				<img class="uth-logo" src="assets/img/uth.png">
			</a>
		</div>
		<div class="collapse navbar-collapse navbaruth" id="bs-example-navbar-collapse-1">
			<ul class="nav navbar-nav">
				<li>
					<a href="" data-toggle="modal" data-target="#about_calc">
						Υπολογισμός Βαθμού
					</a>
				</li>
				<li>
					<a href="" data-toggle="modal" data-target="#autofill">
						Αυτόματη Συμπλήρωση
					</a>
				</li>
				<li class="dropdown">
					<a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">Επιλογή Τμήματος <span class="caret"></span></a>
					<ul class="dropdown-menu">
                        <li><a href="./index.php">Μηχανικών Πληροφορικής Τ.Ε. (ΠΠΣ)</a></li>
						<li><a href="./nps.php">Μηχανικών Πληροφορικής Τ.Ε. (ΝΠΣ)</a></li>
						<li class="disabled"><a href="#"><strike>Μηχανικών Ηλεκτρονικής Τ.Ε.</strike></a></li>
     				</ul>
        		</li>
     		 </ul>
      		<ul class="nav navbar-nav navbar-right">
				<!-- <li><a href="http://appix.org">Appix</a></li> -->
				<li>
					<a href="" data-toggle="modal" data-target="#about">
						About
					</a>
				</li>
				<li class="dropdown">
					<a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">Σχετικοί Σύνδεσμοι <span class="caret"></span></a>
                    <ul class="dropdown-menu">
						<li>
							<a target="_blank" href="https://www.uth.gr/">
								Πανεπιστήμιο Θεσσαλίας
							</a>
						</li>
						<li>
							<a target="_blank" href="https://sis-web.uth.gr/">
								Ηλεκτρονική Γραμματεία
							</a>
						</li>
						<li>
							<a target="_blank" href="https://eclass.uth.gr/">
								e-Class
							</a>
						</li>
						<li role="separator" class="divider"></li>
						<li>
							<a target="_blank" href="http://inf.teiste.gr/">
								Τμήμα Μηχανικών Πληροφορικής
							</a>
						</li>
						<li>
							<a target="_blank" href="http://www.eln.teiste.gr/">
								Τμήμα Ηλεκτρονικών Μηχανικών
							</a>
						</li>
					</ul>
       			</li>
			</ul>
		</div><!-- /.navbar-collapse -->
	</div><!-- /.container-fluid -->
</nav>
